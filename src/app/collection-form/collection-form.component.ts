...
export class CollectionFormComponent implements OnInit {
  ...
  checkEditMode() {
    ...
    if (collectionId && collectionId !== '-1') {
      this.isEditing = true; this.collectionId = collectionId;
      this.collectionService.getCollectionById(collectionId).forEach((collection) => {
        if (collection) { this.collectionForm.patchValue({ name: collection.name, description: collection.description }); }
      });
    }
  }
  ...
  onSubmit() {
    ...
    if (this.isEditing) {
      const updated: Collection = { id: this.collectionId, name: formValue.name, description: formValue.description, creationDate: new Date(), journals: [], creator_email: '' };
      this.collectionService.updateCollection(updated);
    } else {
      const newCollection: Collection = { id: generateUniqueId(), name: formValue.name, description: formValue.description, creationDate: new Date(), journals: [], creator_email: '' };
